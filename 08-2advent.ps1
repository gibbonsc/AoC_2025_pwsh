<# https://adventofcode.com/2025/day/8
  Part 2
  Continue connecting the boxes until all disparate
    clusters are connected into one big cluster.
    Identify the last two boxes that finished that task.
  Author: C Gibbons
#>
...
$elvesWorking = $sortedPairs.Name
$elvesWorkingIndex = 0
$allConnected = $false

[bool[]]$connected = 1..$maxConnections | Foreach-Object { $false } # indexes 
...
do {
    $pair = $elvesWorking[$elvesWorkingIndex++]
    ...
    # check for one unified circuit with all boxes connected
    if ($circuits.Count -ne 1) {
        continue  # no, not unified
    }
    else {
        $allConnected = $true
        foreach ($u in $connected) {
            if (-not $u) {
                $allConnected = $false #no, not all boxes
            }
        }
    }
...
Write-Host "Last pair indexes: $j, $k (multiply X from those two junction boxes)"
$jBoxes[$j],$jBoxes[$k]
"Product of X coordinates: $(($jBoxes[$j].X) * ($jBoxes[$k].X))"
