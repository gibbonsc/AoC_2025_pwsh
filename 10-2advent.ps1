<# https://adventofcode.com/2025/day/10
  Part 2
  Optimized (minimum) button presses to reach an exact configuration of a list of nonnegative integers
  Author: C Gibbons (with help from Microsoft 365 Copilot to craft the dot-sourced utility funcitons)
#>
#[CmdletBinding()]
. ./Linear01DiophantineSystems.v1p4.ps1

#$inp = Get-Content .\10-example.txt
#$inp = Get-Content .\10-2example1.txt
#$inp = Get-Content .\10-2example2.txt
#$inp = Get-Content .\10-2example3.txt
#$inp = Get-Content .\10-inputsorted.txt
...
Function Convert-MachineString {
    ...
    $buttonStrings = $mss[1..($numButtons)]  # buttons in-between
    # $buttonStrings = $mss[1..($numButtons)] | Sort-Object -Property Length
    # sorting by length ensures first "odometer" discovery has fewest aggregate presses
    ...
    foreach ($i in 0..($numButtons - 1)) {
        ...
        $tlen = $buttonStrings[$i].Length
        $ts = $buttonStrings[$i].Substring(1, ($tlen - 2))
        [int[]]$bs = $ts.Split(',')
        $buttons += , @($bs)
        #Write-Debug ( "[i={0,2}] {1}" -f $i, ($bs -join ',') )
    }
...
Function Get-AllPressCandidates {
    [CmdletBinding()]param(
        [int[][]]$ButtonRel,
        [int[]]$JoltageTargets
    )

    # build row-echelon matrix
    $rows = $JoltageTargets.Count
    $cells = $ButtonRel.Count + 1
    $M = [int[][]]@()
    (1..$rows) | Foreach-Object {
        $row = [int[]]::new($cells)
        $M += , @($row)
    }
    for ($b = 0; $b -lt $ButtonRel.Count; $b++) {
        foreach ($d in $ButtonRel[$b]) {
            $M[$d][$b] = 1
        }
    }
    for ($d = 0; $d -lt $JoltageTargets.Count; $d++) {
        $M[$d][$ButtonRel.Count] = $JoltageTargets[$d]
    }
    #$P = Get-DiophantineParameterization -Matrix $M -NonnegativeVars
    #$Sols = Enumerate-DiophantineSolutions -Matrix $M -NonnegativeVars -ReturnAsObjects
    $S = Enumerate-DiophantineSolutions -Matrix $M -NonnegativeVars -ReturnAsObjects -UseGlobalBoundsOnly
    Show-Solutions -InputObject $S -AsCsv
}
...
Function Step-ThroughPressCandidates {
    [CmdletBinding()]param(
        [int[][]]$Bconns,
        [int[]]$jTs  # constrain by joltage targets
    )
    # reduce search space by finding reasonable button press ceilings and floors
    [int[]]$pushCeiling = @()
    foreach ($b in $Bconns) {
    ...
    [int[][]]$jConns = ConvertTo-Dual $Bconns
    [int[]]$pushFloor = @(0) * ($BConns.Count)
    #$loners = @{}  # which joltages can only be set by one button?
    # $spreadMin = [int]::MaxValue  # find smallest joltage that can be set by multiple buttons?
    for ($i=0; $i -lt $jConns.Count; $i++) {
        $d = $jConns[$i]
        if ($d.Count -eq 1) {
            $pushFloor[$d[0]] = $jTs[$i]  # already know minimum times to push this
            #$loners[$d[0]] = $jTs[$i]
        }
        # else {
        #     if ($jTs[$i] -lt $spreadMin) { $spreadMin = $jTs[$i] }
        # }
    }
    
    # build a custom odometer iterator
    $c = $pushFloor.Count; $odometer = [int[]]::new($c); [System.Array]::Copy($pushFloor,$odometer,$c)
    ...
    while ($underflow) {
        ...
        if ($inPlay) {
            ...
            if ($op) {
                $pressCount = ($odometer | Measure-Object -Sum).Sum  # found one!
                # if ($pressCount -lt $maxPresses) {
                #     $maxPresses = $pressCount
                # }
                Write-Debug ("Collected: {0} < {1}" -f $pressCount,($odometer -join ','))
...
Function Invoke-MachineTrials {
    ...
    for (
        ...
        $solutions = Get-AllPressCandidates $buttons $js
        $solutionSums = $solutions | ForEach-Object { ($_.Split(',') | Measure-Object -Sum).Sum }
        $solutionMin = ($solutionSums | Measure-Object -Minimum).Minimum
        Write-Debug "`tSolution: $solutionMin"
        Write-Output $solutionMin
    }
}

#if ($MyInvocation.InvocationName -eq '.') { Invoke-MachineTrials $machineStrings -Debug }
#if ($MyInvocation.InvocationName -eq '.') { Invoke-MachineTrials $machineStrings -Debug | measure -sum }
if ($MyInvocation.InvocationName -eq '.') { 
    $LotsOfWork = Invoke-MachineTrials $machineStrings -Debug
    $LotsOfWork | measure -sum
}
